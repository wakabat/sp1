    ...
    fn lw(&mut self, rd: RiscRegister, _rs1: RiscRegister, _imm: u64) {
        ...
        if self.tracing() {
            dynasm! {
                ...
                // Sign-extend
                // For some reasons, `movsxd Rq(TEMP_B), Rd(TEMP_B)` does not
                // give correct result.
                shl Rq(TEMP_B), 32;
                sar Rq(TEMP_B), 32
            }
    ...
    fn sd(&mut self, _rs1: RiscRegister, rs2: RiscRegister, _imm: u64) {
        ...
        if self.tracing() {
            dynasm! {
                ...
                // Pack clock & value into xmm15
                movq xmm15, rdx;
                pinsrq xmm15, Rq(TEMP_B), 1;

                // Write MemValue to memory
                movdqu [Rq(TEMP_A)], xmm15;
                jmp >done;

                unconstrained:;
                pextrq QWORD [Rq(TEMP_A) + 8], xmm15, 1;
